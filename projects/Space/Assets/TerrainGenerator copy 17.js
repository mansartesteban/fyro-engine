...
class TerrainGenerator {
  ...
  createWater() {
    ...
    let sortedVertices = this.#verticesDatas.map((vertice, index) => ({vertice, index})).sort((a, b) => b.vertice.altitude - a.vertice.altitude)
    console.log("so", sortedVertices)
      for (let sortedVertice of sortedVertices) {
      // for (let i = 0 ; i < this.#verticesDatas.length ; i++) {
        ...
        let lowestNeighborgIndex = neighbors[0];
        for (let j = 0; j < neighbors.length; j++) {
          if (this.#verticesDatas[neighbors[j]].altitude < this.#verticesDatas[lowestNeighborgIndex].altitude) {
            lowestNeighborgIndex = neighbors[j]
          }
        }

        // let currentAltitude = this.#verticesDatas[i].altitude
        // let neighborgAltitude = this.#verticesDatas[lowestNeighborgIndex].altitude

        // if (neighborgAltitude < currentAltitude) {
          this.#verticesDatas[lowestNeighborgIndex].water += this.#verticesDatas[i].water - ((this.#verticesDatas[lowestNeighborgIndex].altitude - this.#verticesDatas[i].altitude) / 10);
        // }
        ...
        // for (let j = 0; j < neighbors.length; j++) {
        //   // const x = this.#geometry.attributes.position.getX(neighbors[j]);
        //   // const y = (this.#geometry.attributes.position.getY(neighbors[j])) * -1;
        //   let alt = this.#verticesDatas[neighbors[j]].altitude// + (this.riverMapNoise(x * .5, y * .5) - .5) * .05
        //   if (
        //     alt < this.#verticesDatas[indexOfLowest].altitude
        //   ) {
        //     indexOfLowest = neighbors[j];
        //     // if (this.#verticesDatas[indexOfLowest].water > 0) {
        //       // break;
        //     // }
        //   }
        // }
        // if (i !== indexOfLowest) {
        //   this.#verticesDatas[indexOfLowest].water += this.#verticesDatas[i].water + (this.#verticesDatas[i].altitude - this.#verticesDatas[indexOfLowest].altitude);
        // } else {
        //   let indexOfLowest = neighbors[0]
          
        //   console.log("là")
        //   let alt = this.#verticesDatas[indexOfLowest].altitude
        //   if (alt - this.#verticesDatas[i].altitude <= this.#verticesDatas[i].water) {
        //     this.#verticesDatas[indexOfLowest].water += this.#verticesDatas[i].water + (this.#verticesDatas[i].altitude - this.#verticesDatas[indexOfLowest].altitude);
        //   }
        //   let sortedNeighbors = neighbors.map((index) => ({index, vertice: this.#verticesDatas[index]}))
        //   sortedNeighbors.sort((a, b) => a.vertice.altitude - b.vertice.altitude)
        //   let min = sortedNeighbors[0]
        //   if (min.vertice.altitude - this.#verticesDatas[i].altitude <= this.#verticesDatas[i].water) {
        //     this.#verticesDatas[min.index].water += this.#verticesDatas[i].water - (this.#verticesDatas[i].altitude - this.#verticesDatas[indexOfLowest].altitude);
        //   }
        // }
        // console.log("alti", this.#verticesDatas[indexOfLowest].water, this.#verticesDatas[i].altitude, this.#verticesDatas[indexOfLowest].altitude)
        // console.log("water", this.#verticesDatas[i].water)
      }

      sortedVertices.reverse()
      let handledNeighborgs = []
      this.tsunami(sortedVertices, handledNeighborgs)
      

        
      //   let indexOfHighest = i;
      //   for (let j = 0; j < neighbors.length; j++) {
      //     let alt = this.#verticesDatas[neighbors[j]].altitude
      //     if (
      //       alt >
      //         this.#verticesDatas[indexOfHighest].altitude
      //     ) {
      //       indexOfHighest = neighbors[j];
      //     }
      //   }
      //   if (indexOfHighest !== i) {
  ...
  tsunami(sortedVertices, handledNeighborgs) {
    for (let sortedVertice of sortedVertices) {

      let i = sortedVertice.index
      
      if (!handledNeighborgs.includes(i)) {
        handledNeighborgs.push(i)
        let capacity = 1 + this.#verticesDatas[i].altitude

        if (this.#verticesDatas[i].water > capacity) {
          
          let overflow = this.#verticesDatas[i].water - capacity
          this.#verticesDatas[i].water = capacity

          let neighbors = this.getNeighbors(
            i,
            this.subdivisions + 1,
            this.subdivisions + 1
          ).filter(neighborg => this.#verticesDatas[neighborg].water < 1 + this.#verticesDatas[neighborg].altitude);

          for (let j = 0; j < neighbors.length; j++) {
            if (!handledNeighborgs.includes(neighbors[j]) && overflow > 0) {
              // this.#verticesDatas[neighbors[j]].water
              let neightbourgCapacity = clamp(1 - this.#verticesDatas[neighbors[j]].water, 0, 1)
              this.#verticesDatas[neighbors[j]].water += Math.min(neightbourgCapacity, overflow)
              overflow -= neightbourgCapacity
              handledNeighborgs.push(neighbors[j])
            }
          }
        }
      ...
      if (this.showAltitude) {
        ...
        //   color.multiplyScalar(blueFactor);
            ...
            // if (this.#verticesDatas[i].water / (maxWater < .1) {
              color.multiplyScalar(this.#verticesDatas[i].water);
            // }
            // }
          ...
          color = this.mixMultipleColors(
            this.#verticesDatas[i].biomes.map((v) => v.biome.color),
            this.#verticesDatas[i].biomes.map((v) => v.influence)
          );
          // color = this.#verticesDatas[i].biome.color || baseColor;
        }
        
        // if (this.#verticesDatas[i].water > 0) {
        //   color = new Color(0x0066FF)
        //   color.multiplyScalar(this.#verticesDatas[i].water / (maxWater / 10));
        // }
