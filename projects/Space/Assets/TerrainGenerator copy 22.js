...
class TerrainGenerator {
  ...
  getNeighbors(index, width, height, size = 2) {
    const neighbors = [];
  ...
  createWater() {
    ...
    for (let sortedVertice of sortedVertices) {
      ...
      // if (this.#verticesDatas[i].altitude > 0.9) {
        this.#verticesDatas[i].water = (1 - this.#verticesDatas[i].altitude * this.#verticesDatas[i].humidity)
      // }
    }
    for (let sortedVertice of sortedVertices) {
      let i = sortedVertice.index;
      ...
      for (let j = 0; j < neighbors.length; j++) {
        if (
          ...
          ||
          this.#verticesDatas[neighbors[j]].altitude - this.#verticesDatas[i].altitude <= this.#verticesDatas[i].water / 50
        ) {
          let deltaHeight =
            (this.#verticesDatas[i].altitude + this.#verticesDatas[i].water) -
            (this.#verticesDatas[neighbors[j]].altitude + this.#verticesDatas[neighbors[j]].water);
  ...
  colorize() {
    ...
    for (
      ...
      if (this.showAltitude) {
        ...
        // let intensity = ((waterAmount / 8) / (maxWater / 3)) ** 2; // Adoucir les bords
        ...
        // } else {
        // if (this.#verticesDatas[i].water > this.#verticesDatas[i].altitude * 10) {
    ...
    for (
      ...
      this.#geometry.attributes.position.setZ(
        ...
        this.#verticesDatas[i].altitude * this.maxAltitude -
          this.#verticesDatas[i].water ** 2 * 2
      );
    }
