...
class TerrainGenerator {
  ...
  lacunarity = 0.34;
  seed;
  seeder = new Alea(this.seed || "seed");
  scene;
  ...
  makeBiomeBoundaries() {
    ...
    for (
      ...
      let gridCoordinates = this.getCoordinateByIndex(i);

      let minJ =
      this.clamp(
        gridCoordinates.x -
          Math.floor(
            this.blendThreshold / (this.terrainWidth / (this.subdivisions + 1))
          ) - 2,
          0,
          this.subdivisions + 1
        );
        ...
        console.log("minmaxJ", minJ, maxJ, gridCoordinates.x)
      ...
      for (let j = minJ; j < maxJ; j++) {
        ...
        this.clamp(
          gridCoordinates.y -
            Math.floor(
              this.blendThreshold /
                (this.terrainHeight / (this.subdivisions + 1))
            ) - 2,
            0,
            this.subdivisions + 1
          );
        ...
        for (let k = minK; k < maxK; k++) {
          let index = this.getIndexByCoordinate(j, k);
          console.log("k", index, j, k)

          const xj = this.#verticesDatas[index].x;
          const yj = this.#verticesDatas[index].y;
  ...
  getIndexByCoordinate(x, y) {
    return y * (this.subdivisions + 1) + x;
  }

  getCoordinateByIndex(i) {
    return {
      x: i % (this.subdivisions + 1),
      y: Math.round(i / (this.subdivisions + 1)),
    };
  }
