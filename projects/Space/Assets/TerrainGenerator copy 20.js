...
class TerrainGenerator {
  ...
  erode(index) {
    let neighbors = this.getNeighbors(index, this.subdivisions + 1, this.subdivisions + 1);
    // neighbors.sort((a,b) => this.#verticesDatas[a].altitude - this.#verticesDatas[b].altitude)
    neighbors = neighbors.filter(neighbor => this.#verticesDatas[neighbor].altitude <= this.#verticesDatas[index].altitude)
    neighbors.sort((a, b) => this.#verticesDatas[b].water - this.#verticesDatas[a].water)

    // neighbors = neighbors.slice(0,4)

    if (neighbors.length) {
      let diff = this.#verticesDatas[index].altitude - this.#verticesDatas[neighbors[0]].altitude
      // diff *= 0.01
      if (diff > 0) {
        this.#verticesDatas[neighbors[0]].water += diff 
        this.#verticesDatas[index].water -= diff/2

        // this.#verticesDatas[index].altitude -= diff 
        this.erode(neighbors[0])
      }
    }
  }
  ...
  colorize() {
    ...
    for (
      ...
      if (this.showAltitude) {
        ...
        // }
        color = this.mixMultipleColors(colors, influences);
